...
class ImageMaskDatasetGenerator:
  ...
  def generate_image_files(self) : 
    ...
    for jpg_file in jpg_files:
      ...
      output_filepath = os.path.join(self.output_images_dir, filename)
      h, w = image.shape[:2]
  ...
  def generate_mask_files(self):
    ...
    for json_file in json_files:
      ...
      i = basename.split(".")[0] 
      image_file = os.path.join(self.images_dir, i + ".jpg")
      with open(json_file, 'r') as f:
        json_data   = json.load(f)
        json_string = json.dumps(json_data, indent=4)
        data        = json.loads(json_string)
        self.parse_json(data, image_file, output_filename, self.output_masks_dir)
        

  def parse_json(self, data, image_file, basename, output_dir ):
      shapes = data["shapes"]
      if shapes == None or type(shapes) !=list:
        return
...
if __name__ == "__main__":
  try:
    ...
    for dataset in datasets: 
      train_images_dir = "./PatchSeg/" + dataset + "-images/"
      train_jsons_dir  = "./PatchSeg/" + dataset + "-labels/"

      output_dir = "./CNSeg-" + dataset + "-master"
